...
from models.Nets import MLP, CNNMnist, CNNCifar, ResNetMnist, ResNetCifar,ResNetSVHN, CNNSVHN
...
def forget_with_generated_data(global_model, generator, device, TARGET_CLASS_INDEX, num_classes,
                               noise_dim=100, num_steps=2000, batch_size=32):
    global_model.train()
    # #cifar+resnet
    ...
    for step in range(num_steps):
        ...
        # # cifar + resnet
        # non_target_labels = torch.randint(0, num_classes, (batch_size,)).to(device)
        non_target_labels = torch.randint(0, num_classes, (batch_size // 2,)).to(device)

        non_target_labels = non_target_labels[non_target_labels != TARGET_CLASS_INDEX]
        non_target_labels = non_target_labels[:batch_size // 2]
...
if __name__ == '__main__':
    ...
    generator = train_generator(generator, net_glob, args.device, TARGET_CLASS_INDEX, num_classes=num_classes,
                                noise_dim=NOISE_DIM, num_steps=2000, batch_size=32)
